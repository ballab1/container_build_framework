...
# --------------------------------              -----------------------------------------
# setup_suite()                                 __test_allsuites_stubbed_sh.mktemp()
# teardown_suite()                              __test_allsuites_stubbed_sh.mock_logger()
# setup()                                       export test_allsuites_stubbed_sh_LOG_file 
# teardown()                                    export test_allsuites_stubbed_sh_UT_TEST_DIR
#                                               export test_allsuites_stubbed_sh_DEBUG
#                                              
#
...
setup_suite() {
    ...
    test_allsuites_stubbed_sh_UT_TEST_DIR="$temporaryDir" 
...
teardown_suite() {
    ...
    [ "$test_allsuites_stubbed_sh_UT_TEST_DIR" ] || return
    [ "$test_allsuites_stubbed_sh_UT_TEST_DIR" != '/' ] || return
    [ "$test_allsuites_stubbed_sh_UT_TEST_DIR" != '~' ] || return
    [ "$test_allsuites_stubbed_sh_UT_TEST_DIR" != "$( cd ~ && pwd )" ] || return
    [ "$test_allsuites_stubbed_sh_UT_TEST_DIR" != "$TMP" ] || return

    # clean up our junk
    rm -rf "$test_allsuites_stubbed_sh_UT_TEST_DIR"
}

# setup MOCK logger
setup() {
    [ "$test_allsuites_stubbed_sh_DEBUG" = 0 ] || printf "\e[94m%s\e[0m\n\n" 'Running setup'
    export LOG=__test_allsuites_stubbed_sh.mock_logger
    test_allsuites_stubbed_sh_LOG_file=$(__test_allsuites_stubbed_sh.mktemp)
    fake 'term.log' '__test_allsuites_stubbed_sh.mock_logger $*'
    touch "$test_allsuites_stubbed_sh_LOG_file" 
}
...
__test_allsuites_stubbed_sh.mktemp() {
    mktemp --tmpdir="$test_allsuites_stubbed_sh_UT_TEST_DIR" $* 2>/dev/null
}
export -f __test_allsuites_stubbed_sh.mktemp

# MOCK logger implementation
__test_allsuites_stubbed_sh.mock_logger() {
    printf "%s\n" "$FAKE_PARAMS" >> "${test_allsuites_stubbed_sh_LOG_file}"
}
export -f __test_allsuites_stubbed_sh.mock_logger

export test_allsuites_stubbed_sh_LOG_file 
export test_allsuites_stubbed_sh_UT_TEST_DIR
export test_allsuites_stubbed_sh_DEBUG=0
