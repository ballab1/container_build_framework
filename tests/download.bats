...
#Tests for download.bashlib
export LOG=test_log

function setup() {
  source "$BATS_TEST_DIRNAME/../cbf/download.bashlib"

  rep_log=.
  script_name=test_log
}

@test "download.getFile" {
  run download.getFile
}

@test "download.getPackages" {
  run download.getPackages
}
